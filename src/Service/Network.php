...
class Network
{
     ...
     */
    public function __construct($code='', $message='', $content='', Message $messageService = null)
    {
        $this->code = $code;
    ...
    public function download($url, $post=array())
    {
        ...
        if ($this->http_code >= 400) {
            ...
            throw new NetworkException(sprintf("The site %s returned a %s error (%s).",
     ...
     * @param integer $code the HTTP Code
     *
     * @return array an array with the HTTP status information
     */
    public function httpData($code)
    {
        ...
        if (array_key_exists($code, $http_codes)) {
            ...
            return array("code" => $code, "message" => "HTTP code not found", "error" => 1);
        }
    }
